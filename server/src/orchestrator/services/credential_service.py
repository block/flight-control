...
async def get_credential(db: AsyncSession, cred_id: str) -> Credential | None:
...
async def update_credential(
    db: AsyncSession, cred_id: str, data: CredentialUpdate
) -> Credential | None:
    cred = await get_credential(db, cred_id)
    if not cred:
        return None
