...
async def register_worker(
    data: WorkerRegisterRequest, db: AsyncSession = Depends(get_db)
):
    worker = await worker_service.register_worker(db, data)
    return WorkerRegisterResponse(id=worker.id, name=worker.name)


@router.post("/heartbeat")
async def worker_heartbeat(
    data: WorkerHeartbeatRequest, db: AsyncSession = Depends(get_db)
):
...
async def poll_for_job(worker_id: str, db: AsyncSession = Depends(get_db)):
    return await worker_service.poll_for_job(db, worker_id)


@router.post("/runs/{run_id}/logs")
async def post_logs(
    run_id: str, data: LogBatchRequest, db: AsyncSession = Depends(get_db)
):
